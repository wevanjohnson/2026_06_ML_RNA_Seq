...
\large
- **WSL** (Windows Subsystem for Linux) lets you run a real Linux environment directly on Windows.
- It gives you a full Unix shell (Bash) so all the commands we learn in this course will work the same as on Mac/Linux.
...
\large
1. In the elevated PowerShell window, run:

```{bash, eval=F}
wsl --install
```

2. This will enable WSL and install **Ubuntu** by default.
3. When the install finishes, **restart your computer**.

## Step 3: Set Up Ubuntu

\large
- After restarting, Ubuntu will launch automatically (or open it from the Start Menu).
- The first launch will take a minute to finalize setup.
- You will be prompted to create:
  - A **UNIX username** (lowercase, no spaces)
  - A **UNIX password** (the cursor will not move while typing — that's normal!)
- You're now ready to use WSL for your projects!

## Accessing WSL

\large
- **From the Start Menu:** search for *Ubuntu* and click to open.
- **From PowerShell or Command Prompt:** simply type `wsl` and press Enter.
- **From RStudio:** open the Terminal pane (Tools \(\rightarrow\) Terminal \(\rightarrow\) New Terminal) — you can configure it to use WSL via Tools \(\rightarrow\) Global Options \(\rightarrow\) Terminal.

\vskip .1in

To access your Windows files from inside WSL, navigate to `/mnt/c/Users/<your-username>/`.
...
The Windows filesystem structure^[**Windows Users:** The typical R installation will make your `Documents` directory your home directory in R. This will likely be different from your home directory in WSL, which lives at `/home/<your-username>/`. Your Windows drives are mounted under `/mnt/` (e.g., `/mnt/c/`).] looks like this:
...
This command is not available in some compact implementations of Unix. Alternatively, we can type the command followed by \texttt{--help}:
