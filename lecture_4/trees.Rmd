...
1. **Build** $B$ trees $T_1, \ldots, T_B$ (made different --- see next slide)
2. **Predict** each case with every tree: $\hat{y}_j$ from tree $T_j$
3. **Combine** the trees:
    - Continuous: average, $\hat{y} = \frac{1}{B}\sum_{j=1}^B \hat{y}_j$
    - Categorical: **majority vote**

## Random forests
\Large
Two sources of randomness make the $B$ trees different:

1. **Bootstrap the rows** --- sample $N$ observations *with replacement* for each tree
2. **Sample the predictors** --- a different random subset of variables ${\bf x}$ is tried for each tree
